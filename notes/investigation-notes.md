# Investigation Notes — Lab 64 Accessibility Features Persistence Investigation

## Investigation Overview

The investigation examined Windows accessibility executables and Registry configuration associated with potential persistence.

The actual accessibility binaries were not modified. A separate lab-only Registry key was used to safely simulate persistence evidence.
...
- Accessibility executables were found in `C:\Windows\System32`.
- All five reviewed accessibility binaries returned valid Authenticode status.
- `utilman.exe` and `sethc.exe` metadata was collected.
- `utilman.exe` SHA-256 was recorded.
- Native IFEO entries were enumerated.
- No suspicious accessibility-specific IFEO configuration was established.
- A lab-only Registry key was created with a simulated debugger value.
- Sysmon Event ID 1 was available.
- Sysmon Event ID 3 was available.
- PowerShell Event ID 4104 did not produce relevant lab evidence.
- Security Event ID 4688 did not provide relevant evidence.
- The lab-only Registry artifact was successfully removed.
- No real accessibility persistence mechanism was created.

## Analyst Assessment

The observed system state was consistent with legitimate Windows accessibility binaries.

The safe simulation successfully demonstrated how an IFEO-style persistence indicator can be represented in Registry evidence without changing the actual Windows accessibility configuration.

The evidence does not establish malicious persistence.
...
```text
Legitimate Accessibility Baseline
+
IFEO Configuration Review
+
Safe Registry Persistence Simulation
+
Telemetry Analysis
+
Cleanup
...
```text
Malicious Accessibility Persistence
```
